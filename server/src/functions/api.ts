...
// Trending endpoint
app.get('/api/trending', function(req: Request, res: Response) {
  const handleRequest = async () => {
    try {
      ...
      const response = createApiResponse(false, undefined, 'Error fetching trending shows', 'Internal server error', 500);
      res.status(500).json(response);
    }
  };
